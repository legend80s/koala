import 'package:flutter/material.dart';
import 'package:myapp/models/image_model.dart';
import 'package:myapp/models/price_model.dart';
import 'package:myapp/models/restaurant_model.dart';

class FoodModel {
  final String name;
  final RestaurantModel store;
  final int orderedCount;
  final PriceModel price;
  final List<ImageModel> images;

  FoodModel({
    @required this.name,
    @required this.store,
    @required this.orderedCount,
    @required this.price,
    @required this.images,
  });

  factory FoodModel.fromJson(Map<String, dynamic> foodJson) {
    return FoodModel(
      name: foodJson['name'],
      orderedCount: foodJson['ordered_count'],
      price: PriceModel.fromJson(foodJson['price']),
      store: RestaurantModel.fromJson(foodJson['store']),
      images: List.from(foodJson['images']).map((image) => ImageModel.fromJson(image)).toList(),
    );
  }
}
